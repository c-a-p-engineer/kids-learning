...
6. UI変更時は `ui/index.html` と整合させること。
7. **画面修正のたびに仕様書を更新**し、実装との差分を残さないこと。
8. 仕様書は **`docs/仕様書テンプレート.md` を共通テンプレート**として使用すること。
9. 仕様書は **1コンテンツ = 1ファイル（`docs/<コンテンツ名>.md`）** で管理し、**`docs/トップ画面.md`** も必ず管理すること。
...
### 3) 仕様書運用（必須）
- 画面/UI/導線に変更がある場合は、同一PR内で `docs/` の該当仕様書を必ず更新する
- 新規コンテンツ追加時は `docs/<コンテンツ名>.md` を新規作成する
- トップ画面変更時は `docs/トップ画面.md` を更新する
- 仕様書の章立ては `docs/仕様書テンプレート.md` に合わせる
